...
- **The authoritative subprocessor list is Annex 3 of the DPA at `sealmetrics.com/dpa`** — check it before writing any subprocessor claim, and never enumerate a list in the docs that contradicts it. As of 2026-08-07 it names Noraina Limited (Ireland — infrastructure and database hosting), Scaleway SAS (Paris — managed LLM inference for Seal AI Private) and Resend (US, under SCCs + DPF — service emails). Cloudflare (CDN/DDoS at the edge) and Hetzner (internal company workloads) touch no customer analytics data, are deliberately absent from Annex 3, and must not be listed as subprocessors.
