...
- **The authoritative subprocessor list is Annex 3 of the DPA at `sealmetrics.com/dpa`** — check it before writing any subprocessor claim, and never enumerate a list in the docs that contradicts it. As of 2026-08-07 it names Noraina Limited (Ireland — infrastructure and database hosting), Scaleway SAS (Paris — managed LLM inference for Seal AI Private) and Resend (US, under SCCs + DPF — service emails). Cloudflare provides CDN/DDoS at the edge but is *not* currently in Annex 3; do not present it as the whole list. Hetzner runs internal company workloads, touches no customer analytics data, and must not be reintroduced as a subprocessor.
- **Seal AI Private runs `gpt-oss-120b`, an Apache-2.0 open-weights model served on Scaleway in Paris** — zero prompt retention, token counters only. **OpenAI is not a subprocessor and prompts never reach it**: the weights are a static public artifact running on Scaleway's infrastructure. "GPT" here is the model's lineage, not the vendor — never write that customer data is sent to OpenAI, and expect readers to assume otherwise. `gemma-4-26b-a4b` and `mistral-small-3.2-24b` were earlier picks, both evaluated and replaced in July 2026; never describe either as current. `docs/lens/seal-ai/model-selection.mdx` is the authority on which model is live and why. BYOK providers are likewise not our subprocessors — that is the customer's own key under their own contract.
