...
- **Seal AI Private runs Gemma 4, an open-weights model self-hosted on Scaleway in Paris** — zero prompt retention, token counters only. **Google is not a subprocessor and prompts never reach it**: the weights run on Scaleway infrastructure. Never write that customer data is sent to Google, and never let "Gemma" read as a Google-operated service. BYOK providers are likewise not our subprocessors — that is the customer's own key under their own contract.
