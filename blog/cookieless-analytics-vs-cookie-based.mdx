...
CNIL's 2020 guidance on cookieless analytics explicitly states that analytics tools without cookies and without storing identifying information can operate under legitimate interest. Sealmetrics' architecture is designed to meet these published criteria. Note that CNIL does not certify, approve, or validate individual analytics tools — see our [CNIL self-assessment](/docs/compliance/cnil-self-assessment) for a criterion-by-criterion analysis.
