...
9. Implementation Technical Guide
...
**Most "cookieless" tools still hash IPs**, creating gray area (learn more in our [Sealmetrics vs Plausible comparison](/blog/sealmetrics-vs-plausible)):
```javascript
// Competitor approach (Plausible, Matomo)
...
- Plausible: Works without consent, but hashes IPs (regulatory risk)
...
```javascript
// Complete Sealmetrics tracking implementation
// Place in <head> of your website

<script>
(function() {
  // 1. Configuration
  const config = {
    siteId: 'YOUR_SITE_ID', // Unique to your website
    apiEndpoint: 'https://api.sealmetrics.io',
    sessionExpiry: 2592000000, // 30 days in milliseconds
  };

  // 2. Generate session ID (random, unique, fresh each visit)
  ...
  function getSession() {
    let session = localStorage.getItem('_seal_session');

    if (!session) {
      session = generateSessionId();
      localStorage.setItem('_seal_session', session);
    }
  ...
  function trackPageview() {
    ...
    // Send via Beacon API (reliable, doesn't block)
    navigator.sendBeacon(config.apiEndpoint + '/track',
  ...
  // 7. Track subsequent navigations (SPA support)
...
| Metric | Google Analytics | Plausible | Matomo | Sealmetrics |
|--------|-----------------|-----------|--------|------------|
| **Data Capture Rate (EU)** | 13-40% | 60-75% | 60-75% | **100%** |
| **Banner Required** | Yes | No | No | **No** |
| **IP Stored** | Yes | Hashed | Hashed | **No** |
| **Consent Required** | Yes | No | No | **No** |
| **Session Expiry** | 2 years | Session-based | 30 mins | **30 days** |
| **GDPR Compliance** | Requires DPA + consent | Built-in | Built-in | **Built-in** |
| **Cookie Rejection Impact** | ~87% data loss | ~20% loss | ~20% loss | **0% loss** |
...
### Step 1: Copy Tracking Snippet

Get your unique snippet from Sealmetrics dashboard:
```html
<script async src="https://cdn.sealmetrics.io/seal.min.js"
  data-site-id="YOUR_UNIQUE_SITE_ID"></script>
```

### Step 2: Place in Website

Add to `<head>` section (or top of `<body>`):
```html
<!DOCTYPE html>
<html>
<head>
  ...
  <!-- Sealmetrics - Place early for best coverage -->
  <script async src="https://cdn.sealmetrics.io/seal.min.js"
    data-site-id="YOUR_UNIQUE_SITE_ID"></script>
</head>
<body>
...
// "Sealmetrics tracking active"

// Or check in Network tab:
// POST requests to api.sealmetrics.io/track
```
...
document.getElementById('contact-form').addEventListener('submit', () => {
  sealmetrics.event('form_submitted', {
    form_type: 'contact',
...
1. Log into Sealmetrics dashboard
2. Go to Site Settings
3. Verify tracking code installed
4. Check Real-Time Analytics (wait 1-2 minutes)
5. See 100% of traffic appearing

**Unlike Google Analytics, you'll see every visitor - no 87% data loss.**
