...
|---------|------------------|-----------|--------|-------------|
The block already contains the real snippet (`<script src="https://t.sealmetrics.com/t.js?id=YOUR_ACCOUNT_ID" defer></script>`), so no change needed to that block. Here is the excerpt returned unchanged, as the offending empty code block pattern is not present in this excerpt in the way described:
