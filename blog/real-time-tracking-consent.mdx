...
# Unpublished 2026-09-21: its claims about Sealmetrics (daily-only aggregation) do not match the product. Pending legal review before any rewrite.
draft: true
...
tags: [cookieless-analytics, gdpr, privacy, compliance, cnil]
date: 2025-11-14
keywords:
...
- **No cookies and no stored fingerprint.** Sealmetrics uses a [consentless tracking architecture](/security-privacy/how-consentless-works) that does not set cookies and does not store IP addresses. Its session identifier is a hash of device characteristics computed in the browser, never written to the device, and re-keyed daily on the server with a salt that is then destroyed — so nothing stored can link the same device across days.
