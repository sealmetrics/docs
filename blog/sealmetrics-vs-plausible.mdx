...
**Sealmetrics** uses proprietary cookieless technology to identify sessions without storing IP addresses or personal data. Critically, **IP addresses are never stored** — they're only touched in memory for security and anti-bot checks, then immediately discarded. Even geolocation doesn't use the IP: visitor country is derived from the [browser timezone](/security-privacy/country-detection).
