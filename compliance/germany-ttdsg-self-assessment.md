...
**Evidence:** The Sealmetrics tracker (~1 KB) writes nothing to the browser. Session continuity (~2-hour window) is handled **server-side**, not by placing any marker on the device — from a session identifier the tracker computes and sends with each hit (see Criterion 2). You can verify this yourself: load a page with Sealmetrics installed and inspect DevTools → Application → Cookies/Storage. Nothing appears.
...
**Assessment:** Sealmetrics' position is that no information *stored* in the terminal equipment is accessed in the sense of §25(1). The runtime properties read and the in-browser hash built from them are disclosed above so that publishers and their DPOs can weigh the interpretive question themselves.
...
German authorities have scrutinized US-linked analytics transfers intensively since *Schrems II*. Sealmetrics avoids the issue entirely: visitor analytics data is never transferred outside the EU.
...
- [TDDDG (formerly TTDSG) — full text (gesetze-im-internet.de)](https://www.gesetze-im-internet.de/ttdsg/)
- [DSK — Orientierungshilfe für Anbieter von Telemedien](https://www.datenschutzkonferenz-online.de/orientierungshilfen.html)
...
- [UK PECR Self-Assessment](/compliance/uk-pecr-self-assessment) — the equivalent self-assessment for the UK
