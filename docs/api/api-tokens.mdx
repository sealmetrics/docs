...
Each token can be granted specific permissions:

| Scope | Description |
|-------|-------------|
| `read` | Read analytics data |
| `write` | Create/update resources |
| `admin` | Full access including user management |
| `accounts:read` | Read account settings |
...
```bash
curl -X GET "https://my.sealmetrics.com/api/v1/stats/overview?account_id=acme" \
...
### 2. Restrict to Specific Accounts

If your integration only needs access to certain accounts:
