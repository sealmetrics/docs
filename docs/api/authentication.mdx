...
| Role | Scopes | Covers |
|------|--------|--------|
| Standard user | `read`, `write` | All `/stats/*`, `/exports/*`, `/batch`, `/sites/*` |
| Admin | `read`, `write`, `admin` | Everything above plus org/member management |
| Read-only | `read` | All `GET` endpoints; rejects writes and exports |

If a request returns `403 insufficient_scope` with a valid JWT, the user's role doesn't include the required scope — switch to a user with the right role, or use an [API key](#api-keys) bound to the site.
