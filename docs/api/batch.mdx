...
tags: [api, batch, queries, performance, agents]
keywords:
  - sealmetrics batch api
  - "post /api/v1/batch multiple queries"
  - "post /api/v1/batch/validate execution order"
  - "depends_on query dependencies batch"
  - "parallel_limit fail_fast batch options"
  - run several stats queries in one request
---
