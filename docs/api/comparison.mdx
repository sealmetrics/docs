...
```http
GET /stats/overview?account_id=my-site&period=30d&compare=previous
```
...
```http
GET /stats/sources?account_id=my-site&period=7d&compare=previous
```
...
```http
GET /stats/sources?account_id=my-site&period=this_month&compare=yoy
```
...
```http
GET /stats/overview?account_id=my-site&start_date=2025-01-01&end_date=2025-01-15&compare=previous
```
...
def get_stats_with_comparison(account_id, period="30d", compare="previous"):
...
```python
def analyze_trend(account_id, periods=["7d", "30d", "90d"]):
    """Analyze trends across multiple time periods."""
    results = {}

    for period in periods:
        data = get_stats_with_comparison(account_id, period, "previous")
