...
tags: [api, ip-allowlist, access-control, security, enterprise]
keywords:
  - sealmetrics ip allowlist api
  - restrict dashboard access by ip address
  - "get /ip-allowlist/check-current"
  - "enforce_for_dashboard lock out warning"
  - bulk import ip cidr patterns
  - "403 ip_not_allowed error"
---
...
:::warning
Enabling `enforce_for_dashboard` without adding your current IP will lock you out. Always test with `enforce_for_api_tokens` first.
:::
...
:::tip
Always use `/check-current` before enabling `enforce_for_dashboard` to ensure you won't be locked out.
:::
