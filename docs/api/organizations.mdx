...
- **Not gated by entitlement.** The response always includes `seal_ai_enabled` so the dashboard can render an appropriate empty state when Seal AI is off.
- **Role-based redaction inside the service**:
  - `member` role → the `by_user` breakdown is omitted; `by_site` only contains sites the caller has access to.
...
```json
{
  ...
  "data": {
    ...
    "daily": [
      { "date": "2026-07-01", "input_tokens": 12000, "output_tokens": 4000 },
      { "date": "2026-07-02", "input_tokens": 15200, "output_tokens": 4900 }
    ],
    "by_user": [
      { "user_id": 42, "email": "alice@acme.com", "input_tokens": 850000, "output_tokens": 280000 }
    ],
    "by_site": [
      { "account_id": "acme", "input_tokens": 620000, "output_tokens": 220000 }
    ]
  }
}
```
...
| Value | Meaning |
|-------|---------|
| `by_user[].email == "LENS (automated)"` | Rows generated by LENS anomaly detection (no user) |
| `by_user[].email == "Former member"` | The user no longer exists in this org |
