...
**First page (default):**
```http
GET /stats/pages?account_id=my-site&period=30d
```

**Second page with 100 items:**
```http
GET /stats/pages?account_id=my-site&period=30d&page=2&page_size=100
```
...
```http
GET /stats/sources?account_id=my-site&sort_by=revenue&sort_order=desc
```

**Lowest bounce rate pages:**
```http
GET /stats/pages?account_id=my-site&sort_by=engaged_entrances&sort_order=desc
```

**Newest conversions first:**
```http
GET /stats/conversions?account_id=my-site&sort_by=count&sort_order=asc
```
...
```http
GET /stats/pages?account_id=my-site&filters=country:eq:ES
```

**EU countries excluding UK:**
```http
GET /stats/sources?account_id=my-site&filters=country:in:ES|FR|DE|IT|PT,country:ne:GB
```

**Product pages from Google:**
```http
GET /stats/pages?account_id=my-site&filters=path:contains:/products/,utm_source:eq:google
```

**Non-branded organic traffic:**
```http
GET /stats/campaigns?account_id=my-site&filters=utm_medium:eq:organic,utm_campaign:not_contains:brand
```

**Mobile users from social:**
```http
GET /stats/pages?account_id=my-site&filters=device_type:eq:mobile,utm_medium:in:social|social-media
```

**Blog pages with regex:**
```http
GET /stats/pages?account_id=my-site&filters=path:regex:^/blog/2025/
```
...
```http
GET /stats/pages?account_id=my-site&segment=paid-traffic&country=ES
```
...
```http
GET /stats/pages?account_id=my-site&utm_medium=cpc&country=ES&filters=path:contains:/products/
```
...
def get_all_pages(account_id, period="30d"):
