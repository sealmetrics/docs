...
Query conversions by custom properties sent with conversion events.
...
Returns all unique property keys used in conversions.
...
{
  "success": true,
  "data": ["brand", "category", "color", "size"]
}
```
...
Get values for a specific property key with metrics.
...
Get pivotable breakdown by property values.
...
response = requests.get(
    ...
    headers={"X-API-Key": "str_your_key"},
    params={
        "account_id": "my-site",
        "period": "30d",
        "sort_by": "revenue",
