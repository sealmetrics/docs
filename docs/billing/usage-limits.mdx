...
**Agent Analytics:** AI agent traffic (bots from OpenAI, Anthropic, Google, etc.) is detected and tracked separately and does **not** count toward your event limit. Agent Analytics is included on all plans.
...
LENS AI uses a **bring-your-own-key (BYOK)** model: you connect your own LLM provider API key (Anthropic, OpenAI, DeepSeek, or Gemini) under **My Account → LLM Providers**. There is **no monthly question quota** imposed by Sealmetrics — questions are billed directly by your chosen provider. See the [AI Assistant docs](/lens/ai-assistant/) for setup.
