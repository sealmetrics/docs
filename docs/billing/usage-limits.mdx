...
**Agent Analytics:** AI agent traffic (bots from OpenAI, Anthropic, Google, etc.) is detected and tracked separately but does **not** count toward your event limit. Agent Analytics is available on Scale and Enterprise plans.
...
3. **Agent Analytics** — AI agent events are already excluded from your event count (Scale+)
...
| Need Agent AI Analytics | Upgrade to Scale |
...
No. Agent Analytics detects AI agent traffic (bots from OpenAI, Anthropic, Google, etc.) and tracks them separately. These events do not count toward your monthly event limit. Note: Agent Analytics is available on Scale and Enterprise plans.
