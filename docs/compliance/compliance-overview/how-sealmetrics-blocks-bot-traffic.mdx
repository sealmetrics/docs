...
- User agents are **not personal data** when they are used as anonymous device-category signals (browser family, OS family, mobile/desktop) rather than as identifiers.
- Sealmetrics stores the user agent string alongside each hit for the standard 24-month retention window purely for aggregated device classification. It is **never linked to any personal identifier** (we don't have one) and **never used to reconstruct a user's history** across sessions.
...
- [Is Sealmetrics GDPR, ePrivacy, CCPA, and PECR Compliant?](/compliance/compliance-overview/is-sealmetrics-privacy-compliant) — the full compliance picture behind the no-IP architecture.
- [Legal FAQ — Sealmetrics Compliance Questions](/compliance/compliance-overview/legal-faq) — why IPs are never used, even for bot filtering.
- [What We Track vs What We Don’t](/security-privacy/what-we-track) — the exact data Sealmetrics does and does not collect.
- [How Sealmetrics determines the country without using IP addresses](/security-privacy/country-detection) — the same no-IP principle applied to geolocation.
