...
tags: [migration, historical-data, getting-started, data-import, migration-wizard]
keywords:
  - migrate from sealmetrics v1 to v2
  - import historical analytics data
  - migration wizard stuck in pending
  - migration date range limit
  - recover old conversions after upgrading
  - migration failed retry
  - migrated traffic shows as unassigned channel
  - no hourly data before migration date
  - 100% bounce rate on migrated data
  - landing pages report empty after migration
  - no device or browser data on migrated traffic
---
...
:::warning Comparing across the seam
