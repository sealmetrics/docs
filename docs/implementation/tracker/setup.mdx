...
To set up tracking, copy your pixel script from **Settings → Pixels** in the dashboard and install it in your site's `<head>`, then verify it under **Pixel Status**. That single pixel collects everything: pageviews automatically, plus conversions via `sealmetrics.conv()` and microconversions via `sealmetrics.micro()` that you call on the relevant pages.
