...
| Tag inside GTM with `Initialization` trigger | Works in practice, but depends on the GTM container loading before the tag that loads the tracker. Use only if you cannot edit the HTML `<head>`. |
...
- [Sealmetrics Dashboard](https://my.sealmetrics.com) — find your Site ID and inspect incoming events.
