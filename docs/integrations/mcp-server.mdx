...
The **SealMetrics MCP Server** lets AI assistants like Claude (Claude Code, Claude Desktop), Cursor, Windsurf, and other MCP clients query your SealMetrics analytics — traffic, conversions, campaigns, and more — in plain natural language. To get it, add one npx-based entry to your client's MCP config (e.g. `npx -y @sealmetrics/mcp` with your API key), and your assistant can start answering questions about your data.

:::tip Don't have an account yet?
This page is the **manual setup** for when you already have a SealMetrics API key. If you want your AI assistant to **create the account for you from the chat** (no key, no terminal), use the one-click [AI Agentic Package (Claude & Codex)](/integrations/agentic-package) instead.
:::
...
- **npm**: [@sealmetrics/mcp](https://www.npmjs.com/package/@sealmetrics/mcp)
