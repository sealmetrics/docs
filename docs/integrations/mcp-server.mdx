...
| Auth | Log in once in the browser (OAuth) | `SEALMETRICS_API_KEY` in the config |
...
The **hosted remote server** is the fastest way to connect. There's nothing to install and nothing to keep updated — you point your AI client at a single URL and log in to SealMetrics once in your browser. It works with Claude, ChatGPT, Cursor, Codex, VS Code, and any client that supports remote (Streamable HTTP) MCP.
...
The first time a client connects, it opens a browser window to sign in to SealMetrics and authorize access. Your existing account permissions apply — the assistant can only see the sites your login can see. No API key is pasted into the client.
...
The first time you use a SealMetrics tool, Claude Code prompts you to authenticate in the browser. You can also run `/mcp` inside Claude Code to check the connection and sign in.
...
4. Click **Add**, then **Connect** and sign in to SealMetrics when the browser opens.
...
4. Choose **OAuth** for authentication, save, and click **Connect** to sign in.
...
Reload Cursor, open **Settings → MCP**, and click **Connect / Sign in** next to SealMetrics to authorize in the browser.
...
Restart Codex. On first use it opens a browser to sign in to SealMetrics.
...
Prefer to run the server on your own machine — for offline setups, or to pin a specific version? The **local server** runs via `npx` and authenticates with your API key instead of the browser. The steps below cover it end to end.
...
### Remote MCP: the browser sign-in didn't complete

Reconnect from your client's connector/MCP settings and complete the SealMetrics login in the browser window it opens. If the window never appears, check that pop-ups aren't blocked, then retry. In Claude Code, run `/mcp` to re-trigger authentication.

### Remote MCP: connected, but no tools or "access denied"

The account you signed in with doesn't have access to the site you're asking about. Sign in with an account that can see it, or ask "list my sites" to confirm which sites are available to your login.
...
**Remote server:** SealMetrics hosts the MCP server at `https://mcp.sealmetrics.com/mcp`. Your client connects to it over HTTPS (Streamable HTTP), you authorize once via browser OAuth, and your existing account permissions decide what the assistant can read. Nothing runs on your machine.
