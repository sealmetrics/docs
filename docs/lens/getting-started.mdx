...
- **Seal AI** — the platform LLM. **No API key needed**; usage counts against your organization's monthly token quota (allocated per plan tier). See your quota anytime in **Seal AI Usage** in the sidebar.
- **Bring-your-own-key (BYOK)** — connect your own provider API key. No per-account quota; you pay your provider directly.

If your organization has Seal AI enabled, it's active by default. To use a BYOK provider, go to **My Account → LLM Providers** and add an API key.
...
|----------|--------------|--------------------------|
| **Seal AI** (platform) | Managed for you | Managed for you |
...
| Google Gemini (BYOK) | `gemini-1.5-flash` | `gemini-1.5-pro` |

Mark one provider as your default and save.

:::info Seal AI quota
Seal AI is scoped by monthly token quota per plan tier (Free / Growth / Scale / Enterprise). When you get close to your limit the dashboard shows an in-app warning; once exceeded, chat calls return a "quota exceeded" error until the next monthly reset. Switch to a BYOK provider (or upgrade the plan) if you need more headroom.
:::
