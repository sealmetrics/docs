...
You can't disable or edit a default rule directly, but you can **override** one. Because your custom rules are evaluated before the defaults, a custom rule that matches the same values as a default rule effectively **replaces it**.

To do this quickly:

1. In the **Default channels** table, click **Override** on the row you want to change.
2. The rule form opens **pre-filled** with the default's patterns.
3. Change only the **Channel name** to the destination you want (or tweak patterns if needed).
4. Test → publish.

Example — you don't want an "AI" channel: click **Override** on the AI defaults → change the channel name to `Referral` (or any other) → publish. All future traffic that would have gone to "AI" now goes to your chosen channel.
