...
Classification happens at session level: the entrance is classified and its pageviews inherit the result. For non-human sessions, Sealmetrics additionally identifies the **automation tool** where possible (`selenium`, `playwright`, `puppeteer`, `browser_use`, `headless_chrome`, `webmcp`) and, via network heuristics, the **AI provider** behind known agents (e.g. OpenAI, Anthropic). See the [agent detection integration](/integrations/agentic-package) for the full signal list.
