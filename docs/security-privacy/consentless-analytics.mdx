...
2. **User Agent** — browser and device type (captured but never stored in analytics)
...
Plausible and Fathom are privacy-focused analytics tools, but they use hashed IP addresses for visitor identification. Some legal experts argue that hashed IPs still constitute personal data under GDPR. Sealmetrics never processes IP addresses for analytics — they are used only for a one-time geolocation lookup and immediately discarded. See our [Sealmetrics vs Plausible comparison](/blog/sealmetrics-vs-plausible) for details.
