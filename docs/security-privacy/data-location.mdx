...
:::note Internal infrastructure (not customer data)
SealMetrics also operates its own infrastructure with Hetzner (Germany/Finland) for **internal company workloads** — development, testing, and internal tooling. This infrastructure does **not** process or store customer analytics data. Earlier versions of this page listed Hetzner in a way that could be misread as customer-data hosting — to be clear: customer analytics data hosting is Dublin, Ireland, exclusively.
:::
...
|-----------|-----------|-------|
| Aggregated reports | 24 months | Monthly/daily aggregations |
| Pageview data | 24 months | Individual hit data |
| Conversion data | 24 months | Including properties |
| Funnel data | 24 months | Multi-touch attribution |
...
SealMetrics keeps its subprocessor list deliberately short. Customer analytics data is stored only in Dublin, Ireland:

| Subprocessor | Role | Data touched | Location |
|--------------|------|--------------|----------|
| Cloudflare | CDN / DDoS protection at the network edge | Traffic in transit (TLS-encrypted requests) | EU edge locations |

Hetzner is **not** a subprocessor of customer analytics data — it hosts SealMetrics' own internal infrastructure only (see the note under [Data Location](#data-location) above).

All subprocessors:

- Sign data processing agreements
- Are GDPR compliant
- Process customer data only in the EU
