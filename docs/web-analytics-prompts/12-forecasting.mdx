...
### WAP-086 — Black Friday forecast based on history
...
### WAP-087 — Pre-Black Friday momentum check
...
### WAP-088 — Black Friday channel mix forecast

```text
Using SealMetrics MCP for site {site_id}, compute the channel share of revenue during Black Friday week of the last 2 years.

Apply that historical share to this year's BF revenue projection (use {projected_bf_revenue} or compute it via WAP-086).
...
### WAP-089 — Black Friday top SKUs prediction
...
### WAP-090 — Post-Black Friday tail forecast
...
### WAP-091 — On-the-books vs pickup curve
...
### WAP-092 — Booking pace alert by date range
...
### WAP-093 — Country mix forecast

```text
For site {site_id} (a hotel), query SealMetrics MCP for the last 24 months.

Compute the historical country mix of bookings for the next quarter (same quarter prior years). Apply that mix to a projected total revenue for next quarter (use {next_q_projection} or run WAP-082 first).
...
### WAP-094 — Campaign trajectory projection
...
### WAP-095 — Goal probability estimator
