...
UTM governance is a small set of written naming rules, applied by everyone who creates a link, that keeps attribution clean. Every marketing report starts with a decision someone made while pasting a link — what to type after `utm_source=`. Get it right consistently and your Sources report tells you exactly which channels make money; get it wrong — `Facebook` here, `facebook` there, `fb` in the ESP — and the same channel splinters into three rows, your channel grouping misfires, and every downstream decision is made on fragmented data.
...
## What happens when a UTM parameter is missing?
...
**Note:**
- Four non-negotiables: always lowercase, one separator (`_` or `-`, never spaces), one canonical name per source and medium, and structured campaign names created in a shared spreadsheet first.
- Sealmetrics reads UTMs from the URL on every page load without cookies, applies UTM Mappings at ingest, then evaluates channel rules (custom first, then default) — so consistent values mean deterministic channels.
- Never put UTMs on internal links: they overwrite the acquisition source mid-visit; use microconversions or Content Grouping instead.
