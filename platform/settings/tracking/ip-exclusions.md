---
title: "IP Exclusions"
description: "Discard tracking hits from specific IPs or CIDR ranges — offices, QA environments, internal tools — so internal traffic never appears in your reports."
canonical_url: "https://docs.sealmetrics.com/platform/settings/tracking/ip-exclusions"
lang: "en"
date_generated: "2026-08-09T18:18:16.203Z"
source_hash: "68faf63e4dee3d7ddb5054168a192d58decd49e6dbf6bb38801741bb5ed07595"
content_type: "documentation"
owner: "docs"
llm_priority: "useful"
source_file: "platform/settings/tracking/ip-exclusions.mdx"
publisher: "SealMetrics"
---

# IP Exclusions

Canonical page: https://docs.sealmetrics.com/platform/settings/tracking/ip-exclusions
...
1. Type the **IP or CIDR range** you want to exclude in the first field. Examples:
   - `203.0.113.7` — a single IPv4 address (your office router's public IP).
   - `198.51.100.0/24` — a /24 IPv4 block (256 addresses).
   - `10.0.0.0/8` — a large private IPv4 block.
   - `2001:db8::1` — a single IPv6 address.
   - `2001:db8::/48` — an IPv6 block.
2. **Optional: add a reason** in the second field (e.g. `Office London`, `QA runs`, `Uptime robot`). The dashboard suggests **Office**, **QA** and **Internal** as one-click options. Reasons are for your own bookkeeping — they don't affect behavior.
3. Click **Add**. The entry is active for **future traffic within ~5 minutes** (the pixel refreshes its per-site cache on that interval).
...
- **100 entries per site.** This is a hard limit. If you need more, you're likely using individual IPs where a CIDR range would work better — a `/24` covers a full office subnet in one entry.
- **IPv4 and IPv6 both supported.** The format is standard CIDR notation. No workaround needed for either.
...
- **They do not restrict dashboard access.** For that, see the [IP Allowlist tab](#important-ip-exclusions-vs-ip-allowlist) next to this one.
- **They don't work across sites.** Exclusions are per-site. If you own multiple sites, you have to configure them on each one.

## Privacy note

Sealmetrics doesn't store visitor IPs in the database. IP Exclusions is the only place the pixel checks the incoming IP in memory — to decide whether to accept the hit or discard it. Either way, the IP is not persisted anywhere in your reports or in the underlying event stream. See [Security & Privacy](/security-privacy) for the full picture.

## Related

- [Channel Grouping](/platform/settings/tracking/channel-grouping) — if you'd rather **tag** internal traffic (e.g. from a shared cloud IP) into a distinct channel instead of discarding it entirely.
- [UTM Mapping](/platform/settings/tracking/utm-mapping) — map your own URL parameters to standard UTMs.
- [Security & Privacy](/security-privacy) — how Sealmetrics handles IPs and other visitor data.
