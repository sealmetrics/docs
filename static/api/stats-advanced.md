...
Canonical page: https://docs.sealmetrics.com/api/stats-advanced
...
| `period` | string | - | Date period (see [Periods](#date-periods)) |
...
| `sort_by` | string | `entrances` | Sort field (see [Sort Fields](#sort-fields)) |
...
Get traffic metrics grouped by UTM source. Same query params as [`/stats/mediums`](#traffic-by-medium), with one addition:
...
Get traffic metrics grouped by UTM campaign. Same query params as [`/stats/mediums`](#traffic-by-medium), with these additions:
...
Get traffic metrics grouped by UTM term (search keywords). Same query params as [`/stats/mediums`](#traffic-by-medium), with these additions:
...
Get traffic by UTM content parameter (ad variations, link types). Same query params as [`/stats/mediums`](#traffic-by-medium), with these additions:
...
| `start_date` / `end_date` / `period` | date / string | - | Date range (see [Date Periods](#date-periods)) |
