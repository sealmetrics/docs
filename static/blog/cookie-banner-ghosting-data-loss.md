...
2. **Session identifier computed**: Sealmetrics JavaScript computes, in the browser, a hash of standard device characteristics (user agent, timezone, languages, screen resolution and similar) plus your site's account ID. That hash is a device fingerprint, but it is not tied to anyone's identity (different visitors with identical devices produce the same value)
...
### Step 2: Validate Your Legal Position
