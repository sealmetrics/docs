...
| Criterion | Sealmetrics Compliance |
|-----------|----------------------|
| Device/browser analysis | ✅ Yes — browser family, OS family, device type |
...
```
Session identifier: a hash of device characteristics computed in the browser, never stored on the device;
  re-keyed on the server with a daily salt destroyed on rotation (live session ~2h, stored pseudonym purged after 1 day)
Session identifier scope: per publisher account — the account is an input to the identifier's derivation, so identifiers cannot match across different publishers' sites
...
| Session-based identification | ✅ No cookie; the stored session identifier changes every day (daily salt, destroyed on rotation) and cannot link days |
| No automatic renewal | ✅ N/A - no persistent storage |

**Technical Implementation:**
```
Storage method: nothing stored on the device (no cookies, no localStorage, no sessionStorage)
Maximum lifetime of the stored identifier: 1 day (daily pseudonym); live session expires after 2h of inactivity
```
...
| Screen resolution, languages | Read, not stored | Session-identifier hash input only | Not stored or reported |
| Timezone | ✅ | Country detection | Country only |
| Session ID | ✅ | Visit counting | Hashed in the browser, re-keyed daily on the server; purged after 1 day |
...
- Stored device fingerprints (the in-browser session hash is re-keyed daily and never stored as sent)
