...
| **Art. 45c(b) TCA — terminal equipment** | ✅ Nothing stored on the device; browser properties read for the session identifier are covered by the information/refusal duties |
...
| Proportionality (data minimization) | ✅ A small set of non-identifying fields per hit ([listed here](/security-privacy/what-we-track)); no IP stored; country-level geo only |
...
| Information read from the device | No cookies or storage are read; standard browser properties (user agent, timezone, languages, screen resolution, colour depth, display preferences, CPU core count, device memory) are read to compute the session-identifier hash, which is never stored as sent |
...
**Assessment:** ✅ There is no storage on the device. Reading browser properties to compute the session identifier is processing on the terminal equipment, and the rule's information/refusal duties are satisfied as set out above — no consent is required under the Swiss model.
