...
**The TCP detail that matters:** the current tracker's 1.1 KB fits in a single packet and arrives in the first round-trip. The v1 tracker's 13.7 KB uncompressed sits right at the edge of TCP's initial congestion window (~14.6 KB, [RFC 6928](https://datatracker.ietf.org/doc/html/rfc6928)), and GA4's ~146 KB needs 4–5 slow-start round-trips. When the visitor is far from the server, every extra round-trip multiplies latency.
...
## Why does the current tracker capture hits the others lose? {#why-the-current-tracker-captures-hits-the-others-lose}
...
- [EasyPrivacy](https://easylist.to/) includes the rule `||sealmetrics.com^$third-party`: visitors with uBlock/AdGuard block both Sealmetrics tracker versions equally. The advantage over GA4 lies in consent and timing, not in ad blockers.
