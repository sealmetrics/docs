...
For online retailers, use the `items` property to send detailed per-product data alongside transaction-level properties:
...
sealmetrics.conv('purchase', 249.97, {
  ...
  customer_type: 'new',
  items: [
    { product_name: 'Wireless Headphones', product_id: 'WH-100', price: 79.99, quantity: 2, category: 'electronics', brand: 'AudioTech' },
    { product_name: 'USB-C Cable', product_id: 'USB-C-3M', price: 89.99, quantity: 1, category: 'accessories' }
  ]
});
```

The `items` array is a native JavaScript array of objects inside the properties parameter. Each object can contain any key-value fields you need (`product_name`, `price`, `quantity`, `category`, `brand`, `variant`, `sku`, etc.). Do not pass `items` as a string or outside the properties object.
...
- Per-product performance and sales volume

To analyze your product items, go to **Properties > Conv. Items** in the dashboard. You'll see each product field (name, category, brand, etc.) broken down by traffic source, medium, and campaign.
...
### Properties Report (Dashboard)

Event properties are available directly in the Sealmetrics dashboard through the **Properties** report. Navigate to your site and click **Properties** in the sidebar.

The Properties report has three modes, selectable via a button group:

| Mode | What it shows |
|------|---------------|
| **Conversions** | Properties from conversion events (e.g., `currency`, `payment_method`, `coupon`) |
| **Microconversions** | Properties from microconversion events |
| **Conv. Items** | Individual product data from the `items` array in purchase conversions |

#### Conv. Items Mode

When you select **Conv. Items**, the report displays per-product analytics from your conversion items. Each product sent in the `items` array appears as a separate entry with all its fields (e.g., `product_name`, `category`, `brand`, `price`, `sku`).

You can:
- **Browse available properties**: See all product fields you've tracked and how often each appears
- **Analyze by attribution**: Select any property (e.g., `product_name`) to see a breakdown by traffic source, medium, and campaign
- **Switch between Totals and Rates**: View raw counts or conversion rates
- **Apply filters**: Filter by conversion type, UTM parameters, channel, country, or device
- **Export to CSV**: Download the data for any mode

**Note:** The `quantity` field is automatically respected — a product with `quantity: 2` counts as 2 units in the report. The `quantity` field itself is not shown as a reportable property since it's used for counting.

**Tip:** When viewing properties in **Conversions** mode, the `items` key is automatically hidden from the property list since its data is better explored in **Conv. Items** mode.

### API Access

You can also query your conversion data programmatically using the Sealmetrics API. This approach suits:
...
Property data retention follows the Sealmetrics data retention policy: conversion data (including properties) is kept for 24 months. Retention is fixed and identical for every plan — it is enforced by database TTLs and is not configurable. See [Data Location & Retention](/security-privacy/data-location).
...
Yes. The **Properties** report in the dashboard lets you explore property data directly. You can select any property key, view its values broken down by traffic source/medium/campaign, and apply filters by conversion type, UTMs, channel, country, or device. For product-level data, use the **Conv. Items** mode to analyze individual items from your purchase conversions.
...
For simple multi-value properties (like multiple categories), you can use a delimiter within the string value:
...
For product-level data, use the `items` array to send structured objects:

```javascript
sealmetrics.conv('purchase', 149.99, {
  currency: 'EUR',
  items: [
    { product_name: 'Headphones', price: 79.99, quantity: 1, category: 'electronics' },
    { product_name: 'Cable', price: 9.99, quantity: 2, category: 'accessories' }
  ]
});
```

The `items` array is a native JavaScript array of objects inside the properties object. Do not pass it as a string.
