...
source_hash: "227931086b1f0d3224fce062ee29f49b21db9c88202e6ef0519aec487b219941"
...
2. Select which UTM field it maps to from the **Maps To** dropdown:
   ...
   - **Term** (`utm_term`)
   - **Content** (`utm_content`)
3. Optionally tick **Override the UTM if the URL already has one** if the URL's own UTMs are set by a platform you don't control (for example a Google Shopping product feed). Off by default.
...
- Changes take about **5 minutes** to apply to new traffic. Previous hits will not be remapped retroactively.
...
- By default a mapping only fills a UTM that the URL doesn't already carry. To override a UTM that is already in the URL, enable **Override the UTM if the URL already has one** on the mapping — see [UTM Mapping → The rules](/platform/settings/tracking/utm-mapping#the-rules).
- Keep your UTM structure consistent across all campaigns to maintain clean and accurate reports.

## Related documentation

- [UTM Mapping](/platform/settings/tracking/utm-mapping) — Full rules, the override option and worked use cases (including Google Shopping feeds)
